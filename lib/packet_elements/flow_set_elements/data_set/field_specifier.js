  ...
  var _construct = function () {
    try {
      ...
      if (cachedFieldLength == 1) {
        ...
        if (shouldParse) {
          try {
            self.fieldValueParsed =
              '' + ByteParser.parseByType(dataType, UINT16)
          } catch (e) {
        ...
        if (shouldParse) {
          try {
            self.fieldValueParsed =
              '' + ByteParser.parseByType(dataType, UINT32)
          } catch (e) {
            self.fieldValueParsed = '' + UINT32
          }
        }
      } else if (cachedFieldLength == 6) {
        self.sizeInBytes = cachedFieldLength
        var macBuffer = buffer.slice(0, cachedFieldLength)

        if (shouldParse) {
          try {
            self.fieldValueParsed = ByteParser.parseByType(dataType, macBuffer)
          } catch (e) {
            try {
              self.fieldValueParsed = macBuffer.toString('hex')
            } catch (er) {
              // No idea what to do with unsupported data ..
        ...
        self.sizeInBytes = 8
        var UINT64 = BigInt(buffer.readBigUInt64BE(0)) // Using BigInt instead of bignum
        self.fieldValue = UINT64.toString()
        if (shouldParse) self.fieldValueParsed = UINT64.toString()
        ...
        if (shouldParse) {
          try {
            self.fieldValueParsed =
              '' + ByteParser.parseByType(dataType, uint8Array)
          } catch (e) {
        ...
        self.sizeInBytes = cachedFieldLength
        try {
          self.fieldValueParsed = buffer
            .slice(0, cachedFieldLength)
            .toString('hex')
        } catch (e) {
          // No idea what to do with unsupported data ..
          self.fieldValueParsed = 'NOT_SUPPORTED'
