                        ...
                        while (index < setDataLength) {
                            ...
                            for (const field of template.FieldSpecifiers) {
                                if (field.fieldLength === 0xFFFF) {
                                    // For variable-length fields, read the length from the data record
                                    const dynamicLength = buffer.readUInt16BE(index);
                                    recordLength += dynamicLength + 2; // Add 2 bytes for the length prefix
                                    index += 2; // Move index past the length field
                                } else {
                            ...
                            if (index + recordLength > setDataLength) {
                                ...
                                console.warn(`Incomplete record detected in data set with template ID ${template.templateId}`);
                                break;
                            }
