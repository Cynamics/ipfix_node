var Netflow7Header = require('./netflow7_header');
var ByteParser = require('../../utils/byte_parsers');

const NETFLOW7_FLOW_SIZE = 48;
...
var NetflowPacket = function (buffer) {
    ...
    var _construct = function (buffer) {
        self.Header = new Netflow7Header(buffer);
        buffer = buffer.slice(self.Header.sizeInBytes);
        while (buffer.byteLength >= NETFLOW7_FLOW_SIZE) {
            self.DataSets.push({
                ...
                l4_src_port: buffer.readUInt16BE(32),
                l4_dst_port: buffer.readUInt16BE(34),
                flags: buffer.readUInt8(36),
                ...
                flow_flags: buffer.readUInt16BE(46),
                router_sc: ByteParser.parseByType("ipv4Address", buffer.readUInt32BE(48)),
            });
            buffer = buffer.slice(NETFLOW7_FLOW_SIZE);
        }
    }
    _construct(buffer);
    return this;
...
NetflowPacket.prototype.NetflowHEader = Netflow7Header;
